...
struct reference_resolver {
    ...
    void resolve_architecture(ast::architecture_body* arch) {
        scope sc(file_lib(arch->my_file));
        apply_unit_use_clauses(sc, arch->packages_in_scope);
        arch->entity_ref = elab.find_entity(sc.lib, arch->primary);
        if(arch->entity_ref) {
            arch->entity_ref->architectures.push_back(arch);
            insert_interfaces(sc, arch->entity_ref->generic_list);
            insert_interfaces(sc, arch->entity_ref->port_list);
            insert_declarations(sc, arch->entity_ref->entity_declarative_items);
        }
